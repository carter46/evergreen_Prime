...
<input class="w-full bg-bg-subtle border border-low rounded-lg py-2 pl-10 pr-4 text-sm text-on-surface focus:ring-1 focus:ring-primary-container focus:border-primary-container outline-none transition-all" placeholder="Search accounts, txids, or users..." type="search" aria-label="Search"/>
</div>
<div class="hidden lg:flex items-center gap-2 px-3 py-1.5 bg-bg-subtle rounded border border-low text-on-surface-variant shrink-0">
...
<span class="absolute -top-1 -right-1 bg-primary-container text-on-primary font-bold text-[10px] min-w-[1rem] h-4 px-1 rounded-full flex items-center justify-center"><?php echo $pendingNotifCount > 99 ? '99+' : $pendingNotifCount; ?></span>
<?php endif; ?>
</button>
...
<p class="font-label-sm text-label-sm text-on-surface font-bold truncate max-w-[140px]"><?php echo htmlspecialchars($adminName); ?></p>
<p class="text-[10px] text-success font-bold uppercase tracking-widest">Verified System</p>
</div>
<?php if ($avatarUrl): ?>
<img alt="" class="w-9 h-9 md:w-10 md:h-10 rounded-full border border-primary-container/30 object-cover shrink-0" src="<?php echo htmlspecialchars($avatarUrl); ?>"/>
<?php else: ?>
<div class="w-9 h-9 md:w-10 md:h-10 rounded-full border border-primary-container/30 bg-surface-container-highest flex items-center justify-center text-primary-container font-bold text-sm shrink-0"><?php echo htmlspecialchars($initials); ?></div>
<?php endif; ?>
</div>
</div>
</header>
