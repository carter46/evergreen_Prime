...
$stmt = $pdo->query('SELECT id, name, slug, plan_type, description, logo_url, min_deposit AS min, max_deposit AS max, yield_min, yield_max, duration_days, min_duration_days, max_duration_days, withdrawal_days, features_json FROM plans WHERE enabled = 1 ORDER BY sort_order, id');
