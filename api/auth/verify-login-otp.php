...
try {
    ...
    $stmt = $pdo->prepare('SELECT id, email, role, two_factor_enabled FROM users WHERE email = ? AND active = 1');
    $stmt->execute([$email]);
    ...
    $twoOn = isset($user['two_factor_enabled'])
        && ($user['two_factor_enabled'] == 1 || $user['two_factor_enabled'] === '1' || $user['two_factor_enabled'] === true);
    if (!$twoOn) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Login verification is not required for this account. Sign in with email and password.']);
        exit;
    }
