...
<p class="font-body-md text-body-md text-on-surface-variant max-w-sm mx-auto md:mx-0">Enter your details to manage your digital assets securely.</p>
</div>
...
<input class="auth-input" id="login-email" name="email" placeholder="name@company.com" type="email" required autocomplete="email"/>
</div>
</div>
<div class="space-y-2">
...
<input class="auth-input pr-12" id="login-password" name="password" placeholder="••••••••" type="password" required autocomplete="current-password"/>
...
<input class="w-5 h-5 rounded border-outline-variant bg-bg-subtle text-primary-container focus:ring-primary-container/30" type="checkbox" name="remember"/>
<span class="font-label-sm text-label-sm text-on-surface-variant">Remember me</span>
</label>
<a class="font-label-sm text-label-sm text-primary-fixed-dim hover:text-primary-container transition-colors" href="/forgot-password">Forgot password?</a>
</div>
<?php if (!empty($_GET['timeout'])): ?>
<div class="text-sm text-primary-container bg-primary-container/10 border border-primary-container/20 px-4 py-3 rounded-lg">You were logged out due to inactivity. Please sign in again.</div>
<?php endif; ?>
...
<span class="font-label-sm text-label-sm text-on-primary uppercase tracking-widest">Sign In to <?php echo htmlspecialchars($siteName); ?></span>
<span class="material-symbols-outlined text-on-primary group-hover:translate-x-1 transition-transform">arrow_forward</span>
</button>
</form>
<div id="login-otp-step" class="space-y-6 hidden">
<h2 class="text-xl font-bold text-on-surface">Verify your identity</h2>
...
<button type="button" id="login-otp-submit" class="w-full bg-primary-container hover:bg-primary-container/90 text-on-primary font-bold py-4 rounded-lg flex items-center justify-center gap-2">
Verify &amp; Sign In
</button>
</div>
...
<p class="mt-8 text-center font-body-md text-body-md text-on-surface-variant" id="login-have-account">
...
© <?php echo date('Y'); ?> <?php echo htmlspecialchars($siteName); ?>. High-risk investment products may not be suitable for all investors.
</p>
</footer>
</div>
